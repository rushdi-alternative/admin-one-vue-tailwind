...
const taskHistoryGroupByDate = ref(null);
const processedTaskHistory = ref(null);
const taskCommentList = ref([]);
const taskAttachmentList = ref([]);
...
async function fetchTask() {
  ...
  try {
    const response = await taskStore.viewTask(apiBaseUrl, taskId);
    task.value = response.data;
    taskHistory.value = response.data.history;
    processedTaskHistory.value = response.data.history;
    taskCommentList.value = response.data.comments;
    taskAttachmentList.value = response.data.attachments;
    ...
    taskHistoryGroupByDate.value = grouped;
    for (const pph in processedTaskHistory.value) {
      removeNullProperties(processedTaskHistory.value[pph])
      removeZeroProperties(processedTaskHistory.value[pph])
    }
  } catch (error) {
...
const submitComment = async () => {
  ...
  if (startDate.value !== task.value.start_date) {
    formData.append('start_date', startDate.value);
  }

  if (endDate.value !== task.value.end_date) {
    formData.append('end_date', endDate.value);
  }

  if (progress.value !== task.value.progress) {
    formData.append('progress', progress.value);
  }

  if (newName.value !== '' && newName.value !== task.value.name) {
    formData.append('name', newName.value);
  }

  if (newDescription.value !== '' && newDescription.value !== task.value.description) {
  ...
  if (
      ...
      startDate.value === task.value.start_date &&
      endDate.value === task.value.end_date &&
      newName.value === task.value.name &&
      newDescription.value === task.value.description &&
      progress.value === task.value.progress
  ) {
    errorMsg.value = "Fill at least one criterion to update the task";
    return;
  }
  // console.log(formData);

  try {
    await taskStore.addCommentToTask(apiBaseUrl, formData, task.value.id);
    newName.value = '';
...
<template>
  <SectionMain>
    ...
    <div v-else>
      <div v-if="task">
        <SectionTitleLineWithButton :icon="mdiBriefcase" :title="task.name" main>
        ...
        <div class="grid grid-cols-1 gap-6 mb-6 ">
          ...
          <CardBox v-for="(history, date) in taskHistoryGroupByDate" :key="date" class="text-justify">
            <div v-for="(h, index) in history" :key="h.id" class="flex flex-row gap-6">
              <div class="flex-1 gap-6 mb-3">
                <div v-show="h.name !== undefined"><b>Task Name</b> - <span class="bold">{{ h.name }}</span></div>
                ...
                <div v-show="h.comment_id !== undefined">
                  <b>Comment</b> - <span class="bold">{{ taskCommentList.find(comment => comment.id === h.comment_id)?.comment_text || 'Comment not found' }}</span>
                </div>

                <div v-if="h.attachment_id !== undefined">
                  <template v-if="taskAttachmentList.find(attachment => attachment.id === h.attachment_id)">
                    <a v-if="isImageFile(taskAttachmentList.find(attachment => attachment.id === h.attachment_id)?.file_name)"
                      :href="apiBaseAssetsUrl + task.id + '/' + taskAttachmentList.find(attachment => attachment.id === h.attachment_id)?.file_name"
                      target="_blank" rel="noopener noreferrer">
                        <div class="image-preview-container">
                          <img :src="apiBaseAssetsUrl + task.id + '/' + taskAttachmentList.find(attachment => attachment.id === h.attachment_id)?.file_name" alt="Attachment" class="image-preview">
                        </div>
                    </a>
                    <a v-else
                      :href="apiBaseAssetsUrl + task.id + '/' + taskAttachmentList.find(attachment => attachment.id === h.attachment_id)?.file_name"
                      download>
                        {{ taskAttachmentList.find(attachment => attachment.id === h.attachment_id)?.file_name }}
                    </a>
                  </template>
                  <span v-else>
                    File not found
          ...
          <CardBox form @submit.prevent="submitComment">
            ...
            <div class="flex flex-rows">
              <FormControl class="flex-1 mb-3" type="textarea" v-model="newComment" placeholder="Add a comment on the task" />
            </div>
            ...
            <template #footer>
              <BaseButtons class="flex justify-end">
                <BaseButton type="submit" color="success" label="Update the Task"  @click="submitComment" />
              </BaseButtons>
            </template>
          </CardBox>
        </div>
      ...
      <div v-else>
        <p class="text-center">Task could not found!!! Please contact the administrator</p>
      </div>
    </div>
  </SectionMain>
</template>
